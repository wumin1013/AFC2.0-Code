...
class MechanismModelMixin:
    ...
    def _legacy_has_prediction_model_ready(self, process_path=None):
    ...
    def _apply_steady_interval_kc_to_sample_df(
        ...
        *,
        ...
        for idx, interval in enumerate(self._get_steady_interval_records(interval_records), 1):
            ...
            state_code = self._resolve_interval_display_state_code(interval)
    ...
    @staticmethod
    def _is_process_row_cutting(row):
        ...
        if mrr_value is not None and mrr_value > 1e-9:
            return True
        ap_value = _safe_float(row.get("ap"))
        ae_value = _safe_float(row.get("ae"))
        feed_value = _safe_float(row.get("feed_effective"))
        return bool(
            ap_value is not None
            and ae_value is not None
            and ap_value > 1e-9
            and ae_value > 1e-9
            and (feed_value is None or feed_value > 1e-9 or move_type == "cutting")
        )

    @staticmethod
    def _collect_process_state_blocks(row_indices, state_codes):
        row_array = np.asarray(row_indices, dtype=int)
        state_array = np.asarray(state_codes, dtype=np.int8)
        if row_array.size == 0 or state_array.size != row_array.size:
            return []
        blocks = []
        block_start = 0
        block_state = int(state_array[0])
        for idx in range(1, row_array.size):
            is_contiguous = row_array[idx] == row_array[idx - 1] + 1
            if is_contiguous and int(state_array[idx]) == block_state:
                continue
            blocks.append((int(block_start), int(idx - 1), int(block_state)))
            block_start = idx
            block_state = int(state_array[idx])
        blocks.append((int(block_start), int(row_array.size - 1), int(block_state)))
        return blocks
    ...
    def _build_profile_segment_records(self, interval_records=None):
        ...
        for row_idx, row in enumerate(self.data):
            if int(row_states[row_idx]) == 2:
                continue
            row_states[row_idx] = 0 if self._is_process_row_cutting(row) else 1

        segment_records = []
        for segment_idx, (block_start, block_end, block_state) in enumerate(
            self._collect_process_state_blocks(row_indices, row_states),
            1,
        ):
    ...
    def _build_compact_runtime_intervals_from_segments(
        ...
        *,
        ...
        for segment in segment_records:
            if not isinstance(segment, dict):
                continue
            state_code = self._resolve_interval_display_state_code(segment)
            if int(state_code) == 0:
                continue
    ...
    def _build_interval_template_sample_kc_array(self, sample_df, profile=None):
        ...
        for segment in segment_records:
            if not isinstance(segment, dict):
                continue
            is_idle_segment = (
                int(self._resolve_interval_display_state_code(segment)) == 1
    ...
    def _legacy_apply_current_interval_mode_kc_override_to_measurement(self):
        if getattr(self, "sample_data_mode", "") != "experiment_measurement":
            return None
    ...
    def _legacy_refresh_manual_measurement_prediction(
        self,
...
# 兼容既有外部导入；主应用使用职责更准确的名称。
PitModelMixin = MechanismModelMixin
