...
class AcademicWorkbenchMixin:
    ...
    def _iter_pit_interval_ranges(self):
        for idx, interval in enumerate(self._get_steady_interval_records(), 1):
    ...
    def refresh_main_pit_preview(self):
        ...
        interval_label = "稳态区间" if self.pit_scope_var.get() == "steady" else "六态区间"
        self.pit_status_var.set(f"PIT 预览: {scope_text}，共 {len(pit_df)} 点，{interval_label} {interval_count} 段")
    ...
    def _resolve_analysis_intervals_from_main_flow(self):
        if bool(
            ...
            try:
                projected = self._get_authoritative_segmentation_sample_records()
            except Exception:
                return []
            return [
                dict(record)
                for record in projected
                if str(record.get("segment_type") or "").strip().lower() == "steady"
            ]
    ...
    def apply_analysis_targets_to_main_flow(self):
        ...
        for record in updated_intervals:
            if not self._record_represents_steady_interval(record):
                continue
            interval_id = str(record.get("zone_id") or record.get("interval_id") or "")
        ...
        steady_updated_intervals = self._get_steady_interval_records(updated_intervals)
        ...
        if not isinstance(getattr(self, "_cached_steady_intervals", None), dict):
            self._cached_steady_intervals = {}
        self._cached_steady_intervals["pit_records"] = [dict(record) for record in steady_updated_intervals]
        ...
        if isinstance(getattr(self, "active_kc_profile", None), dict) and not imported_forward_lock:
            self.active_kc_profile["pit_records"] = [dict(record) for record in steady_updated_intervals]
